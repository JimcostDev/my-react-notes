...
export default function CounterWithEffect({ title,}: CounterProps) {
  const [count, setCount] = useState(0);
  // efectos secundarios (llamadas a APIs, suscripciones, logs, temporizadores, etc)
  useEffect (() => {
    console.log (`El contador ${title} ha cambiado. Nuevo valor: ${count}`);
  }, [count]) // dependencia
